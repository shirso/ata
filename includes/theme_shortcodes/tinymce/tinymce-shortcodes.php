	...
	class Cherry_TinyMCE_Shortcodes {
		...
		function init() {
			...
			if ( ( current_user_can( 'edit_posts' ) || current_user_can( 'edit_pages' ) ) && get_user_option( 'rich_editing' ) == 'true' && ( in_array( $pagenow, array( 'post.php', 'post-new.php', 'page-new.php', 'page.php' ) ) ) )  {
				...
				wp_enqueue_style( 'tinymce-shortcodes', get_bloginfo('template_url'). '/includes/theme_shortcodes/tinymce/tinymce-shortcodes.css', false, CHERRY_PLUGIN_VERSION, 'all' );
			}
		}
		...
		function filter_mce_external_plugins( $plugins ) {
			...
			$plugins['CherryTinyMCEShortcodes'] = wp_nonce_url( esc_url(get_bloginfo('template_url'). '/includes/theme_shortcodes/tinymce/editor-plugin' . $suffix . '.js' ), 'cherry-tinymce-shortcodes' );
		...
		public function output_dialog_markup () {
			...
			$plugin_url = get_bloginfo('template_url'). 'includes/theme_shortcodes/tinymce/'; ?>
			...
			<div id="dialog" style="display:none">
				...
				<div class="clear"></div>
				<script type="text/javascript" id="cherry-shortcode-dialog" src="<?php echo esc_url(get_bloginfo('template_url'). '/includes/theme_shortcodes/tinymce/dialog-js.php' ); ?>"></script>
			</div><!-- #dialog (end) -->
	<?php }
	}
